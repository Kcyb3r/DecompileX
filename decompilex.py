...
DEX2JAR_PATH = "tools/dex-tools-v2.4/d2j-dex2jar.sh"  # or .bat for Windows
...
class ToolInstaller:
    ...
    def install_dex2jar(self):
        ...
        url = "https://github.com/pxb1988/dex2jar/releases/download/v2.1/dex-tools-2.1.zip"
        ...
        try:
            ...
            session = requests.Session(); session.proxies.clear(); response = session.get(url, headers=headers, stream=True, verify=True)
            ...
            try:
                ...
                session = requests.Session(); session.proxies.clear(); response = session.get(alt_url, headers=headers, stream=True, allow_redirects=True)
                response.raise_for_status()
    ...
    def install_jadx(self):
        ...
        session = requests.Session(); session.proxies.clear(); response = session.get(url)
        with open(zip_path, 'wb') as f:
